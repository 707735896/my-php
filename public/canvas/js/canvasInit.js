$(function() {
	var map = [{
		...
		id: 'toggle',
		node: '<div class="img-center" id="toggle"><i class="iconfont icon-up"></i></div>'
	}]

	var arrayh = ['input_text', 'chooseFont', 'line', 'square', 'circle', 'chooseSize', 'clear', 'rubber', 'chooseColor', 'cancelNext'];
	//json数组
	var array = ['toggle', 'pencil', 'enlarge', 'narrow', 'dragID'];
	//		array.sort(function() {
	//			return Math.random() > 0.5 ? -1 : 1;
	//		});
	for(i = 0; i < arrayh.length; i++) {
		for(j = 0; j < map.length; j++) {
			if(arrayh[i] == map[j].id) {
				$("#tools-hide").append(map[j].node);
			}
	...
	for(i = 0; i < array.length; i++) {
		for(j = 0; j < map.length; j++) {
			if(array[i] == map[j].id) {
				$("#tools-normal").append(map[j].node);
			}
	...
	$('#subject').udraggable({
		containment: [-960, -995, 0, 0]
	});
	drag(this);
	$('#container').removeClass("pencil circle rubber square line text move").addClass("move");
	$("#dragID").removeClass("border_nochoose");
	$("#dragID").addClass("border_choose");
	$("#toggle").click(function() {
		$("#tools-hide").toggleClass("set-opacity");
		if($(this).find('i').hasClass('icon-up')) {
			$(this).find('i').removeClass('icon-up').addClass('icon-zhankai');
		} else {
			$(this).find('i').removeClass('icon-zhankai').addClass('icon-up');
		}
	});
	//canvas宽高自适应浏览器窗口
